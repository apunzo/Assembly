# Antonio Punzo
# Homework #5 -Arrays and Loops
# Oct. 15, 2014
# Base program for Homework #6 
# Register legend
...
exp:	.word	0x7f800000
signf:	.word	0x00ffffff
signf2:	.word	0x00800000
signf3:	.word	0x01000000	
...
main:	
	...
	#li	$t0,0x80000000		# load mask to register $t0
	#li	$t1,0x7f800000		# load mask to register $t1
	#li	$t2, 0x00ffffff		# load mask to register $t2
	#li	$t3, 0x00800000		# load mask to register $t3
	lw	$t0, 8($s0)
	and	$s4,$s2,$t0		# Extract sign bit
	lw	$t0, 12($s0)
	and	$s5,$s2,$t0		# Extract exponent
	srl	$s5,$s5,24		# Shift exponent to LSB
	lw	$t0,16($s0)
	...
	#li	$t3,0x01000000		# load mask to register $t3
	lw	$t0, 8($s0)
	and	$t4,$s3,$t0		# Extract sign bit
	lw	$t0, 12($s0)
	and	$t5,$s3,$t0		# Extract exponent
	srl	$t5,$t5,24		# Shift exponent to LSB
	lw	$t0, 16($s0)
